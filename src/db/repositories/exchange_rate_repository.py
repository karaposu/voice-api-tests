...
from db.exchange_rates import ExchangeRates


exchange_client = ExchangeRates(api_key="f76057debb5215fbad5bc6b3e72ea349")
...
class ExchangeRateRepository:
    ...
    def get_or_update_exchange_rate(self, date: str, currency: str) -> dict:
        ...
        if exchange_rate_record:
            ...
            res = exchange_client.fetch(date, currency, cover_rare_exceptions_with_average=True)
            
            # fetched_rates = self.fetch_exchange_rates_from_api(date, currency)
            new_exchange_rate = HistoricalCurrencyRates(
                date=date_obj,
                currency=currency,
                rate_to_usd=  res.rate_to_usd,
                usd_to_gold_rate= res.usd_to_gold_rate ,
                usd_to_chf_rate=res.usd_to_chf_rate
            )
