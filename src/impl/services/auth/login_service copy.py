...
class LoginService:
    ...
    def _create_jwt_for_user(self, user_id: int) -> str:
        ...
        respecting ACCESS_TOKEN_EXPIRE_MINUTES.
        """
        ...
        encoded_jwt = jwt.encode(
            self._add_exp_to_payload(payload, access_token_expires),
            SECRET_KEY,
            algorithm=ALGORITHM
        )
        ...
        try:
            ...
            with session() as db:
                ...
                if user_settings:
                    ...
                    logger.debug(f"UserSettings not found for user_id={user_id}. Skipping login_time_log insertion.")
        except Exception as e:
            logger.error(f"Error inserting login log: {e}")
            # Not re-raising here because a login log is optional; 
            # but you could raise or do session.rollback() if desired.
